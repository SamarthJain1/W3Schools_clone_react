  ...
  return (
    ...
    <About/>
      <div className='w-full'>
        {/* <Link to={"/code"}><button>Submit</button></Link> */}
        {dataRecognized.map((Element, index) => (
          <div key={Element.Title} style={{ backgroundColor: backgroundColors[index % backgroundColors.length] }}>
            <div className='lg:p-[6rem] md:p-[2rem] p-[2rem] md:flex'>
              <div className='w-full'
                style={{
                  color: getButtonTextColor(backgroundButtonColors[index % backgroundButtonColors.length]),
                }}
              >
                <h1 className='lg:text-[90px] md:text-[50px] text-[40px] font-bold text-center mb-4'>{Element.Title}</h1>
                <p className='text-center md:text-[16px] text-[10px] font-semibold mb-4 mx-16'>{Element.description}</p>
                <div>
                  {Element.buttonsEncountered.map((item, buttonIndex) => (
                    <div className='flex justify-center' key={buttonIndex}>
                      <button
                        ...
                        style={{
                          color: getTextColor(backgroundButtonColors[buttonIndex % backgroundButtonColors.length]),
                          backgroundColor: backgroundButtonColors[buttonIndex % backgroundButtonColors.length],
                        }}
                      >
                        {item}
                      </button>
              ...
              <div className='w-full bg-[#E7E9EB] shadow-xl rounded-[4px]'>
                <div className='p-8'>
                  <h1 className='text-[1.5rem] font-semibold mb-5'>{Element.Example}</h1>
                  <div className='bg-white border-l-[4px] border-[#04AA6D]'>
                    <pre className='p-4'>{Element.code.join('\n')}</pre>
                  </div>
                  <a href={`${Element.linksEmbedded}`}>
                    <button className="font-bold text-white bg-[#04AA6D] mt-4 md:text-lg lg:text-[18px] py-2 px-4 w-48 md:w-60 lg:w-60 rounded-3xl relative">{Element.data}</button></a>
                </div>
              </div>
            </div>
          </div>
